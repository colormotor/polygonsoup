...
args.add_argument('--device', type=str, default='/dev/tty.usbserial-110',
                 help='''Serial device''')
...
args.add_argument('--ox', type=float, default=160, help='Origin x (mm)')
args.add_argument('--oy', type=float, default=400, help='Origin y (mm)')
args.add_argument('--pu', type=float, default=-32,
                 help='''Pen up distance (lower value is lower)''')
args.add_argument('--pd', type=float, default=-44,
...
class GrblDevice(threading.Thread):
            ...
            while self.chunks:
                ...
                for line in lines:
                    ...
                    while sum(c_line) >= RX_BUFFER_SIZE-1 | s.inWaiting() :
                        print('In Waiting:')
                        print((sum(c_line), RX_BUFFER_SIZE-1))
                        ...
                        if out_temp.find(b'ok') < 0 and out_temp.find(b'error') < 0 :
                            ...
                            if out_temp.find(b'error') >= 0 and out_temp.find(b'ok') < 0:
                                print(str(grbl_out))
                            else:
                                print('result:')
                                print(str(grbl_out))
    ...
    def draw_path(self, P, newchunk=True):
        ...
        for i, p in enumerate(P):
            if not i:
                self.chunks[-1].append('G0 X%.3f Y%.3f'%(p[0], p[1]))
                #self.pen_down(False)
            else:
                self.chunks[-1].append('G1 X%.3f Y%.3f F2700.0'%(p[0], p[1]))
        #self.pen_up(False)
        if newchunk:
            lock.release()
